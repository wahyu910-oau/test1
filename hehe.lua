...
local function getParryingDagger()
    ...
    if backpack then
        for _, tool in ipairs(backpack:GetChildren()) do
            if tool.Name == "Parrying Dagger" or tool.Name:lower():find("parrying") then
                return tool
            end
        end
    end
    ...
    if char then
        for _, tool in ipairs(char:GetChildren()) do
            if tool:IsA("Tool") and (tool.Name == "Parrying Dagger" or tool.Name:lower():find("parrying")) then
                return tool
            end
        end
    end
...
local function findParryClient()
    for _, c in ipairs(ReplicatedStorage:GetDescendants()) do
        if c:IsA("RemoteEvent") and (c.Name == "ParryClient" or c.Name:lower():find("parry")) then
            return c
        end
    end
    return nil
...
local function isAttacking(killerChar)
    ...
    for _, track in pairs(animator:GetPlayingAnimationTracks()) do
        ...
        if id:lower():find("attack") or id:lower():find("swing") or id:lower():find("hit") then
            return true
        end
    end
    return false
...
local function triggerParry()
    ...
    if dagger then
        ...
        if hum then hum:EquipTool(dagger); dagger:Activate(); return true end
    end
    return false
end
...
local function checkAndParry()
    ...
    if killer and isAttacking(killer.Character) then
        lastParryTime = tick()
        triggerParry()
    end
end
...
local function startAutoGen()
    ...
    autoGenRunning = true
    task.spawn(function()
        while autoGenRunning and featureStates.AutoGenerator do
            doGenerator()
            task.wait(0.2)
        end
...
local function stopAutoGen() autoGenRunning = false end
local function toggleAutoGen(s) featureStates.AutoGenerator = s; if s then startAutoGen() else stopAutoGen() end end
...
local function isZombie(model)
    ...
    if model:FindFirstChild("TheCureStaff") or model:FindFirstChild("CureStaff") or model:FindFirstChild("Handle") or model:FindFirstChild("Flask") then return true end
...
local function scanZombies()
    for _, obj in ipairs(Workspace:GetDescendants()) do
        if obj:IsA("Model") and isZombie(obj) then
...
local function startZombieESP()
    ...
    task.spawn(function()
        while zombieEspRunning and featureStates.ZombieESP do
            scanZombies()
            task.wait(0.25)
        end
        zombieEspRunning = false
    end)
end
...
local function firstBasePart(inst)
    ...
    if inst:IsA("Model") and inst.PrimaryPart and validPart(inst.PrimaryPart) then return inst.PrimaryPart end
...
local Window = Rayfield:CreateWindow({
    Name = "KyelHub - Violence District",
    Icon = 93825530139213,
    LoadingTitle = "KyelHub",
...
MovementSection:CreateToggle({Name="Walk Speed", CurrentValue=false, Flag="ws", Callback=function(s) featureStates.WalkSpeed = s; if s then if speedHumanoid then setWalkSpeed(speedHumanoid, featureStates.WalkSpeedValue); bindSpeedLoop() end else unbindSpeedLoop(); if speedHumanoid then setWalkSpeed(speedHumanoid, 16) end end end})
MovementSection:CreateSlider({Name="Walk Speed Value", Min=0, Max=200, Default=16, Flag="wsv", Callback=function(v) featureStates.WalkSpeedValue = v; if featureStates.WalkSpeed and speedHumanoid then setWalkSpeed(speedHumanoid, v) end end})
MovementSection:CreateToggle({Name="Noclip", CurrentValue=false, Flag="noclip", Callback=function(s) featureStates.Noclip = s; setNoclip(s) end})
MovementSection:CreateToggle({Name="God Mode", CurrentValue=false, Flag="godmode", Callback=function(s) featureStates.GodMode = s; if s then enableGodMode() else disableGodMode() end end})
MovementSection:CreateToggle({Name="MoonWalk", CurrentValue=false, Flag="moonwalk", Callback=function(s) toggleMoonwalk(s) end})

local UtilitySection = PlayerTab:CreateSection("Utilities")
UtilitySection:CreateToggle({Name="Anti AFK", CurrentValue=false, Flag="antiafk", Callback=function(s) featureStates.AntiAFK = s; if s then startAntiAFK() else stopAntiAFK() end end})
UtilitySection:CreateToggle({Name="No Fall", CurrentValue=false, Flag="nofall", Callback=function(s) if s then enableNoFall() else disableNoFall() end end})
UtilitySection:CreateToggle({Name="Anti Stun", CurrentValue=false, Flag="antistun", Callback=function(s) toggleAntiStun(s) end})

-- Survivor Tab
local SurvivorSection = SurvivorTab:CreateSection("Survivor Main")
SurvivorSection:CreateToggle({Name="Auto Parry", CurrentValue=false, Flag="autoparry", Callback=toggleAutoParry})
SurvivorSection:CreateSlider({Name="Parry Range", Min=3, Max=20, Default=10, Flag="parryrange", Callback=function(v) featureStates.ParryRange = v end})
SurvivorSection:CreateSlider({Name="Parry Cooldown (detik)", Min=0.2, Max=2, Default=0.5, Flag="parrycd", Callback=function(v) featureStates.ParryCooldown = v end})

local GenSection = SurvivorTab:CreateSection("Auto Generator")
GenSection:CreateToggle({Name="Auto Generator", CurrentValue=false, Flag="autogen", Callback=toggleAutoGen})
GenSection:CreateDropdown({Name="Generator Quality", Options={"Normal","Instant","Perfect"}, CurrentOption="Perfect", Flag="genquality", Callback=function(v) setGenQuality(v) end})

local OtherSection = SurvivorTab:CreateSection("Other Features")
OtherSection:CreateToggle({Name="Auto Lever", CurrentValue=false, Flag="autolever", Callback=function(s) featureStates.AutoLever = s; if s then startAutoLever() else stopAutoLever() end end})
OtherSection:CreateToggle({Name="No Skillcheck", CurrentValue=false, Flag="noskillcheck", Callback=function(s) featureStates.NoSkillcheck = s; if s then setupNoSkillcheck() else disableNoSkillcheck() end end})
OtherSection:CreateToggle({Name="Bypass Gate", CurrentValue=false, Flag="bypassgate", Callback=function(s) setBypassGate(s) end})

-- Killer Tab
local KillerSection = KillerTab:CreateSection("Killer Utilities")
KillerSection:CreateToggle({Name="ESP Semua Survivor", CurrentValue=false, Flag="killersurvivesp", Callback=function(s) featureStates.SurvivorESP = s; for _,p in ipairs(Players:GetPlayers()) do if p~=LP and getRole(p)=="Survivor" then applyPlayerESP(p) end end end})
KillerSection:CreateButton({Name="Teleport ke Survivor Terdekat", Callback=function()
    ...
    if nearest then LP.Character.HumanoidRootPart.CFrame = nearest.Character.HumanoidRootPart.CFrame + Vector3.new(0,3,0); safeNotify("Teleport", "Teleported to "..nearest.Name,2) else safeNotify("Teleport", "No survivor found!",2) end
end})

-- ESP Tab
local ESPSection = ESPTab:CreateSection("ESP Settings")
ESPSection:CreateSlider({Name="ESP Fill Transparency", Min=0, Max=1, Default=0.7, Flag="espfill", Callback=function(v) featureStates.ESPFillTransparency = v; for _,p in ipairs(Players:GetPlayers()) do if p~=LP then applyPlayerESP(p) end end end})
ESPSection:CreateSlider({Name="ESP Outline Transparency", Min=0, Max=1, Default=0, Flag="espoutline", Callback=function(v) featureStates.ESPOutlineTransparency = v; for _,p in ipairs(Players:GetPlayers()) do if p~=LP then applyPlayerESP(p) end end end})
ESPSection:CreateSlider({Name="ESP Text Size", Min=8, Max=20, Default=14, Flag="esptext", Callback=function(v) featureStates.ESPTextSize = v; for _,p in ipairs(Players:GetPlayers()) do if p~=LP then applyPlayerESP(p) end end end})

local PlayerESPSection = ESPTab:CreateSection("Player ESP")
PlayerESPSection:CreateToggle({Name="Survivor ESP", CurrentValue=false, Flag="survesp", Callback=function(s) featureStates.SurvivorESP = s; for _,p in ipairs(Players:GetPlayers()) do if p~=LP and getRole(p)=="Survivor" then applyPlayerESP(p) end end end})
PlayerESPSection:CreateToggle({Name="Killer ESP", CurrentValue=false, Flag="killeresp", Callback=function(s) featureStates.KillerESP = s; for _,p in ipairs(Players:GetPlayers()) do if p~=LP and getRole(p)=="Killer" then applyPlayerESP(p) end end end})
PlayerESPSection:CreateToggle({Name="Zombie ESP", CurrentValue=false, Flag="zombieesp", Callback=toggleZombieESP})
PlayerESPSection:CreateToggle({Name="Nametags", CurrentValue=false, Flag="nametags", Callback=function(s) featureStates.Nametags = s; for _,p in ipairs(Players:GetPlayers()) do if p~=LP then applyPlayerESP(p) end end end})
PlayerESPSection:CreateToggle({Name="Distance ESP", CurrentValue=false, Flag="distesp", Callback=function(s) featureStates.DistanceESP = s; if s then startDistUpdate() else stopDistUpdate() end; for _,p in ipairs(Players:GetPlayers()) do if p~=LP then applyPlayerESP(p) end end end})
PlayerESPSection:CreateToggle({Name="Survivor Items ESP", CurrentValue=false, Flag="itemsesp", Callback=function(s) featureStates.SurvivorItemsESP = s; for _,p in ipairs(Players:GetPlayers()) do if p~=LP and getRole(p)=="Survivor" then applyPlayerESP(p) end end end})
PlayerESPSection:CreateColorPicker({Name="Survivor Color", Color=featureStates.SurvivorColor, Flag="survcolor", Callback=function(c) featureStates.SurvivorColor = c; for _,p in ipairs(Players:GetPlayers()) do if p~=LP and getRole(p)=="Survivor" and featureStates.SurvivorESP then applyPlayerESP(p) end end end})
PlayerESPSection:CreateColorPicker({Name="Killer Color", Color=featureStates.KillerColor, Flag="killercolor", Callback=function(c) featureStates.KillerColor = c; for _,p in ipairs(Players:GetPlayers()) do if p~=LP and getRole(p)=="Killer" and featureStates.KillerESP then applyPlayerESP(p) end end end})
PlayerESPSection:CreateColorPicker({Name="Zombie Color", Color=featureStates.ZombieColor, Flag="zombiecolor", Callback=function(c) featureStates.ZombieColor = c end})
PlayerESPSection:CreateColorPicker({Name="Survivor Items Color", Color=featureStates.SurvivorItemsColor, Flag="itemscolor", Callback=function(c) featureStates.SurvivorItemsColor = c; for _,p in ipairs(Players:GetPlayers()) do if p~=LP and getRole(p)=="Survivor" and featureStates.SurvivorESP and featureStates.SurvivorItemsESP then applyPlayerESP(p) end end end})

-- World Tab
local WorldSection = WorldTab:CreateSection("World ESP Toggles")
WorldSection:CreateToggle({Name="Generators", CurrentValue=false, Flag="genesp", Callback=function(s) featureStates.GeneratorESP = s; if s and not worldThread then startWorldLoop() end end})
WorldSection:CreateToggle({Name="Hooks", CurrentValue=false, Flag="hookesp", Callback=function(s) featureStates.HookESP = s; if s and not worldThread then startWorldLoop() end end})
WorldSection:CreateToggle({Name="Gates", CurrentValue=false, Flag="gateesp", Callback=function(s) featureStates.GateESP = s; if s and not worldThread then startWorldLoop() end end})
WorldSection:CreateToggle({Name="Windows", CurrentValue=false, Flag="windowesp", Callback=function(s) featureStates.WindowESP = s; if s and not worldThread then startWorldLoop() end end})
WorldSection:CreateToggle({Name="Pallets", CurrentValue=false, Flag="palletesp", Callback=function(s) featureStates.PalletESP = s; if s and not worldThread then startWorldLoop() end end})

local WorldColorSection = WorldTab:CreateSection("World ESP Colors")
WorldColorSection:CreateColorPicker({Name="Generator Color", Color=featureStates.GeneratorColor, Flag="gencolor", Callback=function(c) featureStates.GeneratorColor = c end})
WorldColorSection:CreateColorPicker({Name="Hook Color", Color=featureStates.HookColor, Flag="hookcolor", Callback=function(c) featureStates.HookColor = c end})
WorldColorSection:CreateColorPicker({Name="Gate Color", Color=featureStates.GateColor, Flag="gatecolor", Callback=function(c) featureStates.GateColor = c end})
WorldColorSection:CreateColorPicker({Name="Window Color", Color=featureStates.WindowColor, Flag="windowcolor", Callback=function(c) featureStates.WindowColor = c end})
WorldColorSection:CreateColorPicker({Name="Pallet Color", Color=featureStates.PalletColor, Flag="palletcolor", Callback=function(c) featureStates.PalletColor = c end})

-- Visual Tab
local VisualSection = VisualTab:CreateSection("Visual Enhancements")
VisualSection:CreateToggle({Name="FullBright", CurrentValue=false, Flag="fullbright", Callback=function(s) toggleFullBright(s) end})
VisualSection:CreateToggle({Name="No Fog", CurrentValue=false, Flag="nofog", Callback=function(s) toggleNoFog(s) end})
VisualSection:CreateToggle({Name="Custom Time of Day", CurrentValue=false, Flag="timeofday", Callback=function(s) setTimeOfDay(s) end})
VisualSection:CreateSlider({Name="Time of Day Value", Min=0, Max=24, Default=14, Flag="timevalue", Callback=function(v) updateTimeOfDay(v) end})

-- Teleport Tab
local TeleportSection = TeleportTab:CreateSection("Teleport Locations")
TeleportSection:CreateButton({Name="Teleport ke Generator Terdekat", Callback=function()
    local nearest, nearestDist = nil, math.huge
    for _, model in ipairs(Workspace:GetDescendants()) do
        if model:IsA("Model") and model.Name == "Generator" then
            local part = model:FindFirstChild("HitBox") or model:FindFirstChildWhichIsA("BasePart")
...
TeleportSection:CreateButton({Name="Teleport ke Gate Terdekat", Callback=function()
    local nearest, nearestDist = nil, math.huge
...
TeleportSection:CreateButton({Name="Teleport ke Hook Terdekat", Callback=function()
    local nearest, nearestDist = nil, math.huge
...
SettingsSection:CreateButton({Name="Save Configuration", Callback=function() safeNotify("Settings", "Configuration saved! (Feature not fully implemented in Rayfield)", 2) end})
SettingsSection:CreateButton({Name="Load Configuration", Callback=function() safeNotify("Settings", "Configuration loaded! (Feature not fully implemented in Rayfield)", 2) end})
